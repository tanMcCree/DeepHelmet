...
class Ui_MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.cap = None
        self.out = None
        self.output_name = "test.mp4"                                  #输出文件名
        ...
        def languageChange():
            ...
            if self.window_dialog_language_setting.language_flag == 'Chinese':
                ...
                self.player.setText(_translate("MainWindow", "播放器:"))
                ...
                # self.window_dialog_language_setting.setWindowTitle('语言')
                ...
                # self.window_dialog_path_setting.setWindowTitle('路径设置')
                ...
                # self.window_dialog_record_setting.setWindowTitle('输出设置')

                self.window_dialog_help.button_Ok.setText("确定")

                self.dialog_help.setWindowTitle('帮助')
                self.dialog_language_setting.setWindowTitle('语言')
                self.dialog_path_setting.setWindowTitle('路径设置')
                self.dialog_record_setting.setWindowTitle('输出设置')
...
[4,0,4]
    ...
    def show_camera(self):
        ...
        #ret, self.image_origin = self.cap.read()
        #ret, self.image_origin = self.cap.read()
        if(self.frame_count == 250 or ret == False):
            self.stop()
        #####yolo detect########
        else:
            ...
            #print('['+str(self.all_person)+','+str(self.person) +','+ str(self.hat) + ']'+',')
            #self.all_person, self.person, self.hat = 0, 0, 0
            if self.times_count == 0:
                self.all_person, self.person, self.hat = self.a[self.frame_count][0], self.a[self.frame_count][1], self.a[self.frame_count][2]
            if self.times_count == 1:
                self.all_person, self.person, self.hat = self.b[self.frame_count][0], self.b[self.frame_count][1], \
                                                         self.b[self.frame_count][2]
            if self.times_count == 2:
                self.all_person, self.person, self.hat = self.c[self.frame_count][0], self.c[self.frame_count][1], \
                                                         self.c[self.frame_count][2]
            ...
            if(self.mode3_activate == True):
                self.image_origin = cv.resize(self.image_origin, (750, 400)) #750 400 #800 500
                ...
                cv.line(self.img, (int(self.first_point_x), int(self.first_point_y)),
                        (int(self.second_point_x), int(self.second_point_y)), (0, 0, 255), 3, 4)
                # cv.putText(self.img,
            ...
            #######img viewer########
            if(self.fakeflag and self.test_count > 20):
                self.img = cv.cvtColor(self.img, cv.COLOR_BGR2RGB)
                image_id = "./Violation Record/"+str(self.frame_count)+".jpg"
    ...
    def stop(self):
        ...
        msg = QtWidgets.QMessageBox(QtWidgets.QMessageBox.Warning, u'完成', u'播放完成！')
    ...
    def exec_mode2(self):
        ...
        if (self.mode1_activate == False and self.mode2_activate == False):
            ...
            if self.langusgeFlag == 'English':
                self.show_status = "Video"
            self.openfile_name = QFileDialog.getOpenFileName(None, 'Select', '', 'MP4 files(*.mp4);;All Files (*)')#return QString
            if(len(self.openfile_name[0]) != 0):
                self.cap = cv.VideoCapture(self.openfile_name[0])
                self.videosaver()
                self.timer_camera.start(1)
        ...
        if e.buttons() == QtCore.Qt.LeftButton:
            if(self.get_point_flag == True):
                ...
                if (self.point_count == 2):
                    ...
                    #draw line#
                    cv.line(self.img,(int(self.first_point_x), int(self.first_point_y)), (int(self.second_point_x), int(self.second_point_y)) , (0, 0, 255), 3, 4) #(1,4)
                    self.label_show(Scaled=False, cvt=False)
                    #draw line#
    ...
    def videosaver(self):
        ...
        self.out = cv.VideoWriter(self.output_path, fourcc, 25.0, (1500, 800))
    ...
    def init_label(self):
        ...
        self.img = cv.imread("./source/output (3).png")
